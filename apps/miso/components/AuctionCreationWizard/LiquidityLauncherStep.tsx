...
import { chainId, useToken } from 'wagmi'
import * as yup from 'yup'

import { useAuctionCreateStore } from '../../lib/store'
import { ILiquidityDetails, liquidityDetailsDefaultValues } from '../../lib/store/createLiquidityDetailsSlice'
import { useAuctionedToken } from '../../lib/store/createTokenDetailsSlice'

const useAuctionData = () =>
  useAuctionCreateStore(({ paymentCurrencyAddress, tokenAmount, tokenSymbol }) => ({
...
export const LiquidityLauncherStep: FC<{ children(isValid: boolean): ReactNode }> = ({ children }) => {
  const { paymentCurrencyAddress, tokenAmount, tokenSymbol } = useAuctionData()
  const setLiquidityDetails = useAuctionCreateStore((state) => state.setLiquidityDetails)
  const paymentToken =
    ...
    <Form {...methods} onSubmit={methods.handleSubmit((data: ILiquidityDetails) => setLiquidityDetails(data))}>
      <Form.Fields>
        <div className="flex flex-col">
          <Switch.Group>
            <Typography weight={700}>Use liquidity launcher</Typography>
        ...
        <div className={classNames('col-span-4', liqLauncherEnabled ? '' : 'opacity-40 pointer-events-none')}>
          <Typography weight={700}>Lockup Period*</Typography>
          <div className="flex">
            <ToggleButtonGroup
              size="sm"
              variant="filled"
        ...
        <div className={classNames('w-full md:w-1/2', liqLauncherEnabled ? '' : 'opacity-40 pointer-events-none', '')}>
          <Typography weight={700}>Liquidity Pair (will consist of)</Typography>
