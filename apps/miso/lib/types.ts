...
import { Amount, Price, Token, Type } from '@sushiswap/currency'
...
export type AuctionCreationWizardInputFormatted = Omit<
  ...
  | 'endPrice'
> & {
  paymentCurrency: Type
  startDate: Date
  endDate: Date
  tokenAmount: Amount<Token>
  tokenSupply?: Amount<Token>
  auctionType: AuctionTemplate
  fixedPrice?: Price<Token, Type>
  minimumTarget?: Amount<Type>
  minimumRaised?: Amount<Type>
  startPrice?: Price<Token, Type>
  endPrice?: Price<Token, Type>
  auctionToken: Token
  accounts: string[]
  amounts: string[]
}
